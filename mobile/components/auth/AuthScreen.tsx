...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthSuccess }) => {
  ...
  const headerSubtitleColor = themeMode === 'dark' ? '#94A3B8' : '#64748B';
  ...
  const handleAuth = async () => {
    ...
    if (mode === 'signup' && (!firstName || !lastName || password !== confirmPassword)) {
      Alert.alert(t('auth.error'), t('auth.fillAllFields'));
      return;
    }
  ...
  const handleSocialLogin = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      if (error) {
        Alert.alert(
          'Social Login',
          `Configuration check: ${error.message || 'Setup required for native intent.'}`
        );
      }
    } catch (e) {
      console.error(e);
        ...
        >
          <View style={styles.headerContainer}>
            ...
            <Text style={[styles.titleText, { color: headerTitleColor }]}>
              Il tuo spazio,{'\n'}
              <Text style={styles.gradientText}>il tuo equilibrio.</Text>
            </Text>

            <Text style={[styles.subtitleText, { color: headerSubtitleColor }]}>
          ...
          {/* Temporarily hidden social login options
             ...
             <View style={styles.socialRow}>
                 <TouchableOpacity style={[styles.socialBtn, { backgroundColor: inputBg, borderColor: inputBorder }]} onPress={() => handleSocialLogin('google')}>
                    <Image 
                  ...
                  <TouchableOpacity style={[styles.socialBtn, { backgroundColor: inputBg, borderColor: inputBorder }]} onPress={() => handleSocialLogin('apple')}>
